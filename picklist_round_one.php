...
<main>
    ...
    <h1 style="text-align:center">Round One Picklist</h1>
    <h4>On behalf of Team 5053 and all of our wonderful sponsors and supporters, we'd like to request the assistance of...</h4>
    <?php
    $sql = "SELECT * FROM PicklistRoundOne WHERE CompID=$CompID ORDER BY autoHGAccuracy DESC, AvgAutoHG DESC, HGTrenchAcc DESC, AvgHGTrench DESC";
    ...
    echo '<table id="roundone" cellspacing="5" cellpadding="8">
        ...
        <td><b>Trench HG Acc</b></td>
        <td><b>Avg Trench HG</b></td>
        </tr>';
    while ($row = mysqli_fetch_array($response)) {
        ...
        $HGTrenchAcc = round($row['HGTrenchAcc'] * 100, 2);
        echo '<tr><td align="center"><b>' .
            ...
            $HGTrenchAcc . '%</td><td align="center">' .
            round($row['AvgHGTrench'], 2) . '</td><td align="center">';
        echo '</tr>';
    }
    echo '</table>';
    ...
    $rowB = mysqli_fetch_array($result);
    ?>
    <button class="btn btn-outline-success btn-block input-group-btn" onclick="exportTableToExcel('roundone', '<?php echo $rowB['CompName']; ?>RoundOne')">Export to Excel</button>
